...
class UserTestCase(TestCase):
    ...
    def helper_get_access_token(self):
        response = self.client.post('/user_management/token/', data={'email': 'james@example.com', 'password': 'LetMeIn123!'})
    ...
    def helper_get_access_token_for_other_user(self):
        response = self.client.post('/user_management/token/', data={'email': "jim@example.com", 'password': 'IWantIn123!'})
        return response.data['access']    

@patch('user_management.views.send_mail')
class RegisterUserAPITests(UserTestCase):
    def test_user_can_be_registered(self, mock_send_mail):
        ...
        response = self.client.post('/user_management/register/', data={'name': 'James Smith', 'email': 'james@example.com', 'password': 'LetMeIn123!'})
    ...
    def test_missing_name_returns_bad_request(self, mock_send_mail):
        response = self.client.post('/user_management/register/', data={'email': 'james@example.com', 'password': 'LetMeIn123!'})
        self.assertEqual(response.status_code, 400)
 
    def test_missing_email_returns_bad_request(self, mock_send_mail):
        response = self.client.post('/user_management/register/', data={'name': 'James Smith', 'password': 'LetMeIn123!'})
        self.assertEqual(response.status_code, 400)
 
    def test_missing_password_returns_bad_request(self, mock_send_mail):
        response = self.client.post('/user_management/register/', data={'name': 'James Smith', 'email': 'james@example.com'})
        self.assertEqual(response.status_code, 400)
...
class VerifyUserEmailAPITests(UserTestCase):
    def test_verification_email_link(self, mock_send_mail):
        self.client.post('/user_management/register/', data={'name': 'James Smith', 'email': 'james@example.com', 'password': 'LetMeIn123!'})
        user = User.objects.get(email='james@example.com')
        response = self.client.post(f'/user_management/email/verify/',data={'verification_secret': user.verification_email_secret})
        user = User.objects.get(email='james@example.com')
    ...
    def test_bogus_verification_does_not_work(self, mock_send_mail):
        self.client.post('/user_management/register/', data={'name': 'James Smith', 'email': 'james@example.com', 'password': 'LetMeIn123!'})
        user = User.objects.get(email='james@example.com')
        response = self.client.post(f'/user_management/email/verify/',data={'verification_secret': '4BCDEFGH1JKLmNOpqrSTUVWXYZ'})
        user = User.objects.get(email='james@example.com')
        self.assertFalse(user.verified_email)
        self.assertFalse(user.is_active)
    ...
    def test_anything_but_post_request_returns_405_error(self, mock_send_mail):
        response = self.client.get('/user_management/email/verify/')
        self.assertEqual(response.status_code, 405)
        response = self.client.put('/user_management/email/verify/')
        self.assertEqual(response.status_code, 405)
        response = self.client.patch('/user_management/email/verify/')
        self.assertEqual(response.status_code, 405)
...
class UserAuthenticationTests(UserTestCase):
    ...
    def test_user_with_bad_password(self):
        self.helper_create_user()
        response = self.client.post('/user_management/token/',data={'email': 'james@example.com', 'password': 'password'})
    ...
    def test_user_login_api_fails_without_email_and_password(self):
        response = self.client.post('/user_management/token/')
        self.assertEqual(response.status_code, 400)
 
    def test_user_login_api_fails_without_email(self):
        response = self.client.post('/user_management/token/',data={'password': 'LetMeIn123!'})
        self.assertEqual(response.status_code, 400)
 
    def test_user_login_api_fails_without_password(self):
        response = self.client.post('/user_management/token/',data={'email': 'jsmith'})
        self.assertEqual(response.status_code, 400)
 
    def test_user_login_api_fails_with_non_existent_email(self):
        response = self.client.post('/user_management/token/',data={'email': 'jsmith', 'password': 'LetMeIn123!'})
        self.assertEqual(response.status_code, 401)
 
    def test_user_login_api_fails_with_get_request(self):
        response = self.client.get('/user_management/token/')
        self.assertEqual(response.status_code, 405)
...
class UserProfileTests(UserTestCase):
    def test_user_can_retrieve_their_profile(self):
        ...
        response = self.client.get('/user_management/profile/', HTTP_AUTHORIZATION=f"Bearer {access_token}")
        self.assertEqual(response.status_code, 200)
...
class UpdateUserProfileTests(UserTestCase):
    def test_unauthenticated_user_cannot_change_name(self):
        ...
        self.helper_create_another_user()
        access_token=self.helper_get_access_token_for_other_user()
        response = self.client.put('/user_management/profile/update/',{'name': 'Jimmy Smyth', 'email': 'jim@example.com'}, content_type="application/json")
        self.assertEqual(response.status_code, 401)
    ...
    def test_user_can_change_name(self):
        ...
        self.helper_create_another_user()
        access_token=self.helper_get_access_token_for_other_user()
        response = self.client.put('/user_management/profile/update/',{'name': 'Jimmy Smyth'}, content_type="application/json", HTTP_AUTHORIZATION=f"Bearer {access_token}")
        self.assertEqual(response.status_code, 200)
    ...
    def test_blank_update_does_not_change_anything(self):
        ...
        self.helper_create_another_user()
        access_token=self.helper_get_access_token_for_other_user()
        response = self.client.put('/user_management/profile/update/', content_type="application/json", HTTP_AUTHORIZATION=f"Bearer {access_token}")
        self.assertEqual(response.status_code, 200)
        user=User.objects.get(email='jim@example.com')
    ...
    def test_user_can_change_password(self):
        ...
        self.helper_create_another_user()
        access_token=self.helper_get_access_token_for_other_user()
        response = self.client.put('/user_management/profile/update/',{'password': 'IWantInNow123!'}, content_type="application/json", HTTP_AUTHORIZATION=f"Bearer {access_token}")
        self.assertEqual(response.status_code, 200)
        response = self.client.post('/user_management/token/', data={'email': 'jim@example.com', 'password': 'IWantIn123!'})
        self.assertEqual(response.status_code, 401)
        response = self.client.post('/user_management/token/', data={'email': 'jim@example.com', 'password': 'IWantInNow123!'})
        self.assertEqual(response.status_code, 200)
    ...
    def test_update_return_new_access_token(self):
        ...
        self.helper_create_another_user()
        access_token=self.helper_get_access_token_for_other_user()
        response = self.client.put('/user_management/profile/update/',{'name': 'Jimmy Smyth', 'password': 'IWantInNow123!'}, content_type="application/json", HTTP_AUTHORIZATION=f"Bearer {access_token}")
        decoded = jwt.decode(response.data['access'], settings.SECRET_KEY, algorithms=["HS256"])
    ...
    def test_anything_but_put_request_returns_error(self):
        ...
        self.helper_create_another_user()
        access_token=self.helper_get_access_token_for_other_user()
        response = self.client.get('/user_management/profile/update/',{'name': 'Jimmy Smyth', 'password': 'IWantInNow123!'}, content_type="application/json", HTTP_AUTHORIZATION=f"Bearer {access_token}")
        self.assertEqual(response.status_code, 405)
        response = self.client.post('/user_management/profile/update/',{'name': 'Jimmy Smyth', 'password': 'IWantInNow123!'}, content_type="application/json", HTTP_AUTHORIZATION=f"Bearer {access_token}")
        self.assertEqual(response.status_code, 405)

@patch('user_management.views.send_mail')
class PasswordResetTests(UserTestCase):
    def test_anything_but_post_request_returns_405_error(self, mock_send_mail):
        response = self.client.get('/user_management/password/forgot/')
        self.assertEqual(response.status_code, 405)
        response = self.client.put('/user_management/password/forgot/')
        self.assertEqual(response.status_code, 405)
        response = self.client.patch('/user_management/password/forgot/')
        self.assertEqual(response.status_code, 405)
    
    def test_no_email_returns_400(self, mock_send_mail):
        self.helper_create_user()
        self.helper_create_another_user()
        response = self.client.post('/user_management/password/forgot/')
        self.assertEqual(response.status_code, 400)

    def test_invalid_email_returns_200(self, mock_send_mail):
        self.helper_create_user()
        self.helper_create_another_user()
        response = self.client.post('/user_management/password/forgot/', data={'email': 'nobody@example.com'})
        self.assertEqual(response.status_code, 200)

    def test_valid_email_returns_200(self, mock_send_mail):
        self.helper_create_user()
        self.helper_create_another_user()
        response = self.client.post('/user_management/password/forgot/', data={'email': 'james@example.com'})
        self.assertEqual(response.status_code, 200)

    def test_valid_email_sends_a_password_reset_email(self, mock_send_mail):
        self.helper_create_user()
        self.helper_create_another_user()
        self.client.post('/user_management/password/forgot/', data={'email': 'james@example.com'})
        user = User.objects.get(email='james@example.com')
    ...
    def test_password_reset_shows_vague_error_on_failure(self, mock_send_mail):
        ...
        self.helper_create_another_user()
        self.client.post('/user_management/password/forgot/', data={'email': 'james@example.com'})
        response = self.client.post('/user_management/password/reset/', data={'password': 'MyNewPasswordIWillNotForget123!', "reset_secret": "1234567890123fdaacdsad"})
        self.assertEqual(response.status_code, 400)
    ...
    def test_password_reset_shows_reset_message_on_success(self, mock_send_mail):
        ...
        self.client.post('/user_management/password/forgot/', data={'email': 'james@example.com'})
        user = User.objects.get(email='james@example.com')
        response = self.client.post(f'/user_management/password/reset/', data={'password': 'MyNewPasswordIWillNotForget123!', "reset_secret": user.reset_password_secret})
        self.assertEqual(response.status_code, 200)
    ...
    def test_password_reset_fails_with_good_secret_but_no_password(self, mock_send_mail):
        ...
        self.client.post('/user_management/password/forgot/', data={'email': 'james@example.com'})
        user = User.objects.get(email='james@example.com')
        response = self.client.post(f'/user_management/password/reset/', data={"reset_secret": user.reset_password_secret})
        self.assertEqual(response.status_code, 400)
    ...
    def test_password_reset_changes_password(self, mock_send_mail):
        ...
        self.client.post('/user_management/password/forgot/', data={'email': 'james@example.com'})
        user = User.objects.get(email='james@example.com')
        self.client.post(f'/user_management/password/reset/', data={'password': 'MyNewPasswordIWillNotForget123!', "reset_secret": user.reset_password_secret})
        response = self.client.post('/user_management/token/', data={'email': 'james@example.com', 'password': 'LetMeIn123!'})
        self.assertEqual(response.status_code, 401)
        response = self.client.post('/user_management/token/', data={'email': 'james@example.com', 'password': 'MyNewPasswordIWillNotForget123!'})
        self.assertEqual(response.status_code, 200)
    ...
    def test_password_reset_fails_with_no_reset_secret(self, mock_send_mail):
        ...
        response = self.client.post('/user_management/token/', data={'email': 'james@example.com', 'password': 'LetMeIn123!'})
        self.assertEqual(response.status_code, 200)
        response = self.client.post(f'/user_management/password/reset/', data={'password': 'MyNewPasswordIWillNotForget123!'})
        self.assertEqual(response.status_code, 400)
        response = self.client.post('/user_management/token/', data={'email': 'james@example.com', 'password': 'LetMeIn123!'})
        self.assertEqual(response.status_code, 200) 
    ...
    def test_password_reset_fails_on_blank_reset_secret(self, mock_send_mail):
        ...
        response = self.client.post('/user_management/token/', data={'email': 'james@example.com', 'password': 'LetMeIn123!'})
        self.assertEqual(response.status_code, 200)
        response=self.client.post(f'/user_management/password/reset/', data={'password': 'MyNewPasswordIWillNotForget123!', "reset_secret": ""})
        self.assertEqual(response.status_code, 400)
        response = self.client.post('/user_management/token/', data={'email': 'james@example.com', 'password': 'LetMeIn123!'})
        self.assertEqual(response.status_code, 200)
